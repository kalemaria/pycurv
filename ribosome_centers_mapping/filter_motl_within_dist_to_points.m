...
%distance (in pixels) to any foreground voxel from a given binary points mask.
% Input:
%   motl            motive list with particles
%   points_mask     mask with single, disconnected voxels = 1, the rest = 0
%   dist            maximal distance in pixels between the particle
...
for part = 1:size(motl, 2) % for each particle (column in motl),
    ...
    min_dist = dist+1;
    for i = (x-ceil(dist)):(x+ceil(dist))
        for j = (y-ceil(dist)):(y+ceil(dist))
            for k = (z-ceil(dist)):(z+ceil(dist))
                if points_mask(i, j, k) == 1 % if there is a point
                    if D(i, j, k) < min_dist % and it's within dist to the particle center
