...
class VectorVotingTestCase(unittest.TestCase):
    ...
    def parametric_test_sphere_curvatures_old(
    ...
    def parametric_test_sphere_curvatures(
            self, radius=5, inverse=False, k=3, g_max=0, epsilon=0, eta=0):
        """
        Runs all the steps needed to calculate curvatures for a test sphere
        ...
        if not os.path.isfile(surf_file):
            sm = SphereMask()
            sphere_mask = sm.generate_sphere_mask(r=radius,
                                                  box=(radius * 2 + 3), t=1)
            run_gen_surface(sphere_mask, '{}sphere_r{}'.format(fold, radius))
        ...
        if inverse:
            reverse_normals = True
        # a graph with normals pointing inwards is generated (positive
        # curvatures, normal case for this surface generation method)
        else:
            reverse_normals = False
        tg.build_graph_from_vtk_surface(surf, verbose=False,
                                        reverse_normals=reverse_normals)
    ...
    #     """
    #     Tests whether curvatures for an inverse sphere with radius 20 are
    #     correctly estimated using NVV with g_max = 3:
    #
    #     kappa1 = kappa2 = -1/20 = -0.05; 30% of difference is allowed
    #     """
    #     self.parametric_test_sphere_curvatures(
    #         radius=20, inverse=True, g_max=3, epsilon=0, eta=0)
