...
by shortest distances along
the graph edges. Uses the last four methods of the TriangleGraph class in the
surface_graphs module, which implement triangle-wise operations of the
algorithm.
...
def vector_voting(tg, k=3, g_max=0.0, epsilon=0, eta=0, exclude_borders=True):
    ...
    # class 1; scaled in nm):
    tg.graph.vp.T_1 = tg.graph.new_vertex_property("vector<float>")
    # vertex property for storing the estimated principal directions of the
    # minimal curvature of the corresponding triangle (if the vertex belongs to
    # class 1; scaled in nm):
    tg.graph.vp.T_2 = tg.graph.new_vertex_property("vector<float>")
    # vertex property for storing the estimated maximal curvature of the
    # corresponding triangle (if the vertex belongs to class 1; scaled in nm):
    tg.graph.vp.kappa_1 = tg.graph.new_vertex_property("float")
    # vertex property for storing the estimated minimal curvature of the
    # corresponding triangle (if the vertex belongs to class 1; scaled in nm):
